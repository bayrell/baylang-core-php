...
Runtime.BaseLayout = class extends Runtime.BaseModel
{
	...
	assets(path)
	{
		let res = Runtime.rtl.getContext().hook(Runtime.Hooks.RuntimeHook.ASSETS, Runtime.Map.create({
			"layout": this,
			"path": path,
		}));
		return res.get("path");
...
Runtime.RenderContainer = class extends Runtime.BaseObject
{
	...
	changeLayout(layout_name)
	{
		...
		/* Save widgets */
		let old_widgets = this.layout ? this.layout.widgets : new Runtime.Map();
		/* Create new layout */
		this.createLayout(layout_name);
		/* Restore widgets */
		let keys = Runtime.rtl.list(old_widgets.keys());
...
if (typeof Runtime == 'undefined') Runtime = {};
if (typeof Runtime.Web == 'undefined') Runtime.Web = {};
Runtime.Web.RenderHelper = class
{
	/**
	 * From rgb
	 */
	...
	{
		let ch = this.substr(color, 0, 1);
		if (ch == "#") color = this.substr(color, 1);
		let r = "";
		let g = "";
		let b = "";
		let sz = this.strlen(color);
		if (sz == 3)
		{
	...
	{
		let result = this.rgbToInt(color);
		let r = result[0];
		let g = result[1];
		let b = result[2];
		...
		if (g < 0) g = 0;
		if (b < 0) b = 0;
		return "#" + String(this.intToRgb(r, g, b));
	}
	...
	static strip_tags(content, allowed_tags)
	{
		...
		if (matches)
		{
			for (let i = 0; i < matches.count(); i++)
			{
				...
				if (tag_match)
				{
					let tag_name = this.strtolower(tag_match[0][2]);
					if (allowed_tags.indexOf(tag_name) == -1)
					{
						content = this.replace(tag_str, "", content);
					}
	...
	static getClassName(){ return "Runtime.Web.RenderHelper"; }
	static getMethodsList(){ return null; }
	static getMethodInfoByName(field_name){ return null; }
	static getInterfaces(){ return []; }
};
...
if (typeof Runtime.Web == 'undefined') Runtime.Web = {};
if (typeof Runtime.Web.Hooks == 'undefined') Runtime.Web.Hooks = {};
Runtime.Web.Hooks.CanonicalUrl = class extends Runtime.Web.Hooks.AppHook
{
	/**
	 * Init params
	 */
	...
	route_before(params)
	{
		let container = params.get("container");
		let layout = container.layout;
		let seo = layout.getWidget("seo");
		if (!seo) return;
		if (!layout.route) return;
		if (layout.route.uri == null) return;
		/* Build canonical url */
		let canonical_url = layout.url(layout.route.name, layout.route.matches, Runtime.Map.create({"domain": false}));
		/* Add get parameters */
		let keys = layout.request_query.keys().sort();
		for (let i = 0; i < keys.count(); i++)
		{
			let key = keys.get(i);
			if (!this.query.has(key)) continue;
			if (this.query.get(key).indexOf(layout.route.name) == -1) continue;
			let value = layout.request_query.get(key);
...
Runtime.Web.Hooks.Components = class extends Runtime.Hooks.RuntimeHook
{
	...
	static hook(items)
	{
		return new Runtime.Entity.Hook(this.getClassName(), Runtime.Map.create({"components": items}));
	}
	...
	static header(items)
	{
		return new Runtime.Entity.Hook(this.getClassName(), Runtime.Map.create({"header": items}));
	}
	...
	static footer(items)
	{
		return new Runtime.Entity.Hook(this.getClassName(), Runtime.Map.create({"footer": items}));
	}
...
Runtime.Widget.AppHook = class extends Runtime.Hooks.RuntimeHook
{
	...
	register_hooks()
	{
		this.register(this.constructor.COMPONENTS, "components", 50);
	}
...
Runtime.Widget.Image = {
	...
	methods:
	{
		...
		render: function()
		{
			...
			{
				/* Element img */
				__v.element("img", new Runtime.Map({"class": rs.className(["image", this.class, componentHash]), "src": this.src, "key": "image"}));
			}
			else
			{
				/* Element div */
...
Runtime.Widget.Input = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element input */
			__v.element("input", new Runtime.Map({"class": rs.className(["input", componentHash]), "type": this.type, "name": this.name, "value": this.getValue(), "placeholder": this.placeholder, "@ref": "input", "onChange": this.onChange, "onKeydown": this.onKeyDown}).concat(props));
...
Runtime.Widget.Link = {
	...
	methods:
	{
		render: function()
		{
			...
			/* Element a */
			let __v0 = __v.element("a", new Runtime.Map({"class": rs.className([this.class, componentHash]), "href": this.href}).concat(attrs));
			__v0.push(this.text);
...
Runtime.Widget.Pagination = {
	...
	methods:
	{
		...
		{
			let uri = this.layout.request_full_uri;
...
Runtime.Widget.Section = {
	...
	getComponentStyle: function(){ return ".section__wrap.h-c82a{max-width: var(--content-max-width);margin-left: auto;margin-right: auto;padding: 0px var(--padding-desktop)}@media(max-width: 1000px){.section__wrap.h-c82a{padding: 0px var(--padding-tablet)}}@media(max-width: 768px){.section__wrap.h-c82a{padding: 0px var(--padding-mobile)}}"; },
	getRequiredComponents: function(){ return new Runtime.Vector(); },
};
