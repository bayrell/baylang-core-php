...
Runtime.Unit.AssertHelper = class
{
	...
	static equalValue(value1, value2, message)
	{
		...
		let value_type1 = Runtime.rtl.getType(value1);
		let value_type2 = Runtime.rtl.getType(value2);
...
Runtime.Unit.TestProvider = class extends Runtime.BaseProvider
{
	...
	{
		let getMethodsList = new Runtime.Callback(class_name, "getMethodsList");
		let getMethodInfoByName = new Runtime.Callback(class_name, "getMethodInfoByName");
		let methods = getMethodsList.apply();
		methods = methods.filter((method_name) =>
		{
			let method_info = getMethodInfoByName.apply(new Runtime.Vector(method_name));
	...
	async runTestMethod(class_name, method_name)
	{
		...
		try
		{
			let callback = new Runtime.Callback(class_name, method_name);
			if (!callback.exists())
			{
